...
export function transformCapacidad(filePath){
    const workbook = XLSX.readFile(filePath);
    const objectCapacidad = ObjectEmpPrPy(workbook);
    return objectCapacidad;
}
  ...
  //Recorrer todas las hojas
    workbook.SheetNames.forEach(element => {
      ...
      if(jsonData[0]){
        ...
        let pr = getPrPyVentas(jsonData,Object.keys(jsonData[0]),element);
...
function getPrPyVentas(jsonData, key,element) {
